...
function overlayTextOnImage($imagePath, $text1, $text2, $text3, $text4, $text5, $fontSize1, $fontSize2, $fontSize3, $fontSize4, $fontSize5) {
   ...
   {
      header('Content-Disposition: attachment; filename="hamawardqsl.png"');
   }
   imagepng($image);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if (preg_match('/^[A-Z0-9\_]+$/', $callsign)) {
      ...
      if (sizeof($qsos) > 0) {
         ...
         overlayTextOnImage('hamawardqsl.png', str_replace("0", "Ø", str_replace("_", "/", $callsign)), shortenString($name, 35), shortenString(ArrayUniqueOrderToString($bands),47), shortenString(ArrayUniqueOrderToString($modes),47), formatDateString($date), 50, 33, 22, 22, 22);
      }
      else {
...
<meta charset="utf-8">
<title>"CALLSIGN" Special Event Amateur Radio Station</title>
...
<body>
	<div id="content">
		<p>
      		<span class="h">"CALLSIGN" Special Event Amateur Radio Station</span><br>
      	</p>
...
Ut wisi enim ad minim veniam, quis nostrud exerci tation ullamcorper suscipit lobortis nisl ut aliquip ex ea commodo consequat. Duis autem vel eum iriure dolor in hendrerit in vulputate velit esse</p>
		...
		<p>Wenn du ein QSO mit "CALLSIGN" gemacht hast, so kannst du hier die QSL-Karte herunterladen</p>
