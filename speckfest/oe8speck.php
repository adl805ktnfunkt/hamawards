...
function overlayTextOnImage($imagePath, $text1, $text2, $text3, $text4, $text5, $fontSize1, $fontSize2, $fontSize3, $fontSize4, $fontSize5) {
   ...
   {
      header('Content-Disposition: attachment; filename="OE8SPECK.png"');
   }
   imagepng($image);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   if (preg_match('/^[A-Z0-9\_]+$/', $callsign)) {
      ...
      if (sizeof($qsos) > 0) {
         ...
         overlayTextOnImage('oe8speck.png', str_replace("0", "Ø", str_replace("_", "/", $callsign)), shortenString($name, 35), shortenString(ArrayUniqueOrderToString($bands),47), shortenString(ArrayUniqueOrderToString($modes),47), formatDateString($date), 50, 33, 22, 22, 22);
      }
      else {
...
<meta charset="utf-8">
<title>OE8SPECK Special Event Amateur Radio Station</title>
...
<body>
	<div id="content">
		<p>
      		<span class="h">OE8SPECK Special Event Amateur Radio Station</span><br>
      	</p>
		...
		<p>Wenn du ein QSO mit OE8SPECK gemacht hast, so kannst du hier die QSL-Karte herunterladen</p>
